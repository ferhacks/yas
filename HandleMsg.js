...
module.exports = HandleMsg = async (aruga, message) => {
        ...
        if (isGroupMsg && !isGroupAdmins){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    aruga.reply(from, `*「 GROUP LINK DETECTOR 」*\nLo siento, links de grupos no estan admitidos :(`, id).then(() => {
	...
	// Filter Banned People
        ...
        case 'randomhentai':
            ...
            const nsfq = body.slice(7)
            const nsfp = await rugaapi.randomNimek(nsfq)
            await aruga.sendImage(from, `${nsfp}`, '', 'Nih...', id)
            .catch(() => {
